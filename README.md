...
📊 Database Relationship Mapping (Core Tables)
The system backend heavily utilizes relational mappings across the following entities:

Vehicle Table: Stores user-specific vehicle models and manufacturing data.

Problem Table: Logs technical diagnostics and error definitions.

Solution Table: Maps the expert solutions attached to specific errors.

UserRatings Table: Tracks customer feedback and safeguards critical solution nodes from being orphaned or deleted accidentally.
...
Open lib/utils/constants.dart and update the base URL with your active backend environment IP address or development tunnel:

Dart
   class AppConstants {
     static const String baseUrl = "[http://192.168.18.52/RyC/api](http://192.168.18.52/RyC/api)";
...
Bash
   flutter run
🔮 Future Roadmap
AI-Powered Diagnostics: Integrating Computer Vision to identify car engine issues via real-time camera streaming.

In-App Live Chat: Implementing WebSockets to connect users directly with live Mechanical/Electrical experts.

Payment Gateway Integration: Adding Stripe/JazzCash/EasyPaisa for expert premium consultations.
