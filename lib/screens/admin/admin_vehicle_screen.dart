...
class _AdminVehicleScreenState extends State<AdminVehicleScreen> {
  ...
  // iOS Flow: Selected cars and their top problems
  Map<String, dynamic> selectedCarInBrand = {}; // Brand -> Selected Car Object
  Map<int, String> selectedCarProblems = {};    // VID -> Problem Title
  ...
  // --- iOS Logic: Fetch Analysis ---
  Future<void> _fetchAnalysis(dynamic car) async {
    int vid = car['vid'];
    // Agar pehle se load nahi hua toh API hit karein
    if (!selectedCarProblems.containsKey(vid)) {
      String? problem = await ApiService.fetchTopRatedProblem(vid);
      if (problem != null) {
        setState(() {
          selectedCarProblems[vid] = problem;
        });
      }
    }
  }
  ...
  Future<void> _loadAllApprovedCars() async {
    ...
    try {
      ...
      setState(() {
        ...
        // Default selection logic like iOS
        var distinctMakes = allApprovedCars.map((c) => c['make'].toString()).toSet().toList();
        for (var make in distinctMakes) {
          var carsInMake = allApprovedCars.where((c) => c['make'] == make).toList();
          if (carsInMake.isNotEmpty) {
            selectedCarInBrand[make] = carsInMake[0];
            _fetchAnalysis(carsInMake[0]); // Pehli gari ka problem load karein
          }
        }
      });
    } catch (e) {
  ...
  // Brand Dropdown with iOS Flow (Problem Analysis)
  Widget _brandDropdown(String brandName, List<dynamic> cars) {
    bool isOpen = openBrand == brandName;
    dynamic currentSelectedCar = selectedCarInBrand[brandName];
    int? currentVid = currentSelectedCar?['vid'];
    String? topProblem = currentVid != null ? selectedCarProblems[currentVid] : null;

    return Column(
      children: [
        GestureDetector(
          onTap: () => setState(() => openBrand = isOpen ? "" : brandName),
          child: Container(
            padding: const EdgeInsets.all(15),
            ...
            child: Row(
              children: [
                ...
                Text(brandName, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                const Spacer(),
                // Dropdown for selecting specific model (iOS Picker style)
                DropdownButton<dynamic>(
                  value: currentSelectedCar,
                  underline: const SizedBox(),
                  icon: const Icon(Icons.keyboard_arrow_down),
                  items: cars.map((car) {
                    return DropdownMenuItem<dynamic>(
                      value: car,
                      child: Text("${car['model']} ${car['variant']}", style: const TextStyle(fontSize: 13)),
                    );
                  }).toList(),
                  onChanged: (newCar) {
                    setState(() {
                      selectedCarInBrand[brandName] = newCar;
                    });
                    _fetchAnalysis(newCar);
                  },
                ),
              ],
            ),
        ...
        if (isOpen)
          Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text("Year: ${currentSelectedCar['year']}", style: const TextStyle(color: Colors.black87)),
                const SizedBox(height: 8),
                // 🔥 Top Problem Display (iOS style)
                if (topProblem != null)
                  Text(
                    "🔥 $topProblem",
  ...
  // --- Rest of your widgets (Tabs, Request List) remain untouched ---
  Widget _buildRequestCarList() {
    if (isLoadingRequests) return const Center(child: CircularProgressIndicator());
    if (pendingCars.isEmpty) return const Center(child: Text("No pending requests."));
