...
class _ExpertAddSolutionViewState extends State<ExpertAddSolutionView> {
  ...
  void _loadInitialData() async {
    ...
    setState(() {
      ...
      if (allVehicles.isNotEmpty) {
        // Unique and sorted list of Makes
        makes = allVehicles
            .where((e) => e['make'] != null)
  ...
  void _applyLocalFilter() {
    setState(() {
      ...
      // Problem Dropdown ko bhi shuru mein khali rakhne ke liye null set kiya
      sProblem = null;
    });
  }

  // Handle Make changes and update downstream selections (No auto-selection)
  ...
  // Handle Model changes and update downstream selections (No auto-selection)
  void _onModelChanged(String v) {
    setState(() {
      ...
      years = allVehicles
          .where((e) => e['make'] == sMake && e['model'] == v)
  ...
  // Handle Year changes and update downstream selections (No auto-selection)
  ...
  void _handleSave() async {
    ...
    if (titleController.text.trim().isEmpty) {
      _showSnackBar("Please enter a solution title!", Colors.red);
      return;
    }
    if (selectedVid == null || sProblem == null) {
      _showSnackBar("Please select Vehicle and Problem details!", Colors.red);
      return;
    }

    // Step verification (iOS style: Description and Image are mandatory)
    for (int i = 0; i < stepsList.length; i++) {
      String desc = stepsList[i]["controller"].text.trim();
      File? img = stepsList[i]["image"];
      if (desc.isEmpty || img == null) {
        _showSnackBar("Please write details and pick an image for Step ${i + 1}", Colors.orange);
        return;
      }
    }
    ...
    // Call updated sequential multi-step upload method
    bool success = await ApiService.uploadExpertSolution(
      expertId: widget.currentExpertId,
      vid: selectedVid!,
      pid: sProblem['pid'],
    ...
    if (success) {
      _showSnackBar("Solution Saved Successfully!", Colors.green);
      widget.onSave();
    } else {
      _showSnackBar("Failed to save. Check Console/Backend.", Colors.red);
    }
  }

  void _showSnackBar(String text, Color bg) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(text), backgroundColor: bg, duration: const Duration(seconds: 3)),
    );
  }
    ...
    return Scaffold(
      ...
      body: isLoading
        ...
        child: Column(
          ...
          children: [
            const Center(
              child: Text("Add New Solution", style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Color(0xFF1B2E4B))),
            ),
            const SizedBox(height: 20),
            _carDropdowns(),
            _radioSelection(),
            const SizedBox(height: 10),
            const Text("Select Problem", style: TextStyle(fontWeight: FontWeight.bold)),
            const SizedBox(height: 5),
            _dropdownBox(filteredProblems.isEmpty ? "No problem found" : "Select Problem", filteredProblems, (val) => setState(() => sProblem = val), sProblem),
            const SizedBox(height: 10),
            const Text("Solution Title", style: TextStyle(fontWeight: FontWeight.bold)),
            const SizedBox(height: 5),
            _solutionTitleField(),
            const SizedBox(height: 25),
            const Text("Steps", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF1B2E4B))),
            const SizedBox(height: 10),
            ...stepsList.asMap().entries.map((entry) => _stepCard(entry.key)).toList(),
            _addStepButton(),
            const SizedBox(height: 30),
  ...
  Widget _carDropdowns() {
    return Column(children: [
      Row(children: [
        Expanded(child: _dropdownBox("Make", makes, (v) => _onMakeChanged(v), sMake)),
        const SizedBox(width: 10),
        Expanded(child: _dropdownBox("Model", models, (v) => _onModelChanged(v), sModel)),
      ]),
      const SizedBox(height: 10),
      Row(children: [
        Expanded(child: _dropdownBox("Year", years, (v) => _onYearChanged(v), sYear)),
        const SizedBox(width: 10),
        Expanded(child: _dropdownBox("Variant", variants, (v) => setState(() { sVariant = v; selectedVid = v['vid']; }), sVariant)),
      ]),
    ]);
  ...
  Widget _radioSelection() => Row(children: [
    Radio(value: "Electrical", groupValue: expertise, onChanged: (v) { setState(() => expertise = v!); _applyLocalFilter(); }),
    const Text("Electrical"),
    const SizedBox(width: 20),
    Radio(value: "Mechanical", groupValue: expertise, onChanged: (v) { setState(() => expertise = v!); _applyLocalFilter(); }),
    const Text("Mechanical")
  ]);

  Widget _solutionTitleField() => Container(
      padding: const EdgeInsets.symmetric(horizontal: 10),
      decoration: BoxDecoration(border: Border.all(color: Colors.grey.shade300), borderRadius: BorderRadius.circular(8)),
      child: TextField(controller: titleController, decoration: const InputDecoration(hintText: "Add solution title", border: InputBorder.none))
  );

  Widget _stepCard(int index) => Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(
          ...
          children: [
            Text("Step ${index + 1}", style: const TextStyle(color: Colors.blue, fontWeight: FontWeight.bold)),
            if (stepsList.length > 1)
              IconButton(
                icon: const Icon(Icons.delete_outline, color: Colors.redAccent, size: 20),
                onPressed: () => setState(() => stepsList.removeAt(index)),
              )
          ],
        ),
        TextField(controller: stepsList[index]["controller"], decoration: const InputDecoration(hintText: "Explain step...", border: InputBorder.none)),
        if (stepsList[index]["image"] != null)
          Padding(
            padding: const EdgeInsets.only(bottom: 8.0),
            child: ClipRRect(
              borderRadius: BorderRadius.circular(6),
              child: Image.file(stepsList[index]["image"]!, height: 120, width: double.infinity, fit: BoxFit.cover),
            ),
          ),
        _pickImageButton(index)
      ])
  );

  Widget _pickImageButton(int index) => ElevatedButton.icon(
      onPressed: () => _pickImage(index),
      icon: const Icon(Icons.camera_alt, size: 16),
      label: const Text("Select Image"),
      style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF4A5C7A), foregroundColor: Colors.white, minimumSize: const Size(double.infinity, 36))
  );

  Widget _addStepButton() => TextButton.icon(
      onPressed: () => setState(() => stepsList.add({"controller": TextEditingController(), "image": null})),
      icon: const Icon(Icons.add),
      label: const Text("Add Step"),
      style: TextButton.styleFrom(backgroundColor: Colors.grey.shade100, minimumSize: const Size(double.infinity, 40))
  );

  Widget _actionButtons() => Row(children: [
    Expanded(child: OutlinedButton(onPressed: widget.onBack, child: const Text("Back"))),
    const SizedBox(width: 15),
    Expanded(child: ElevatedButton(onPressed: _handleSave, style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF1B2E4B)), child: const Text("Save", style: TextStyle(color: Colors.white))))
  ]);
